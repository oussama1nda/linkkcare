...
let medicines = [
  { id: 1, name: 'Paracétamol',  dose: '500 mg',   freq: '3x par jour', time: '08:00', comp: 'A', notes: 'Avec de la nourriture', taken: true,  ic: 0 },
  { id: 2, name: 'Amoxicilline', dose: '1000 mg',  freq: '2x par jour', time: '12:00', comp: 'C', notes: '',                      taken: false, ic: 1 },
  { id: 3, name: 'Vitamine D',   dose: '1000 UI',  freq: '1x par jour', time: '07:30', comp: 'E', notes: 'À jeun',                taken: false, ic: 2 },
];
      ...
      <div class="med-card" id="mc-${m.id}">
        ...
        <div class="med-body">
          ...
          <div class="med-row">
            ...
            <span class="med-comp">Cpt. ${m.comp}</span>
          </div>
...
// ===========================
//  COMPARTIMENT SELECTOR
// ===========================

function selectComp(btn) {
  document.querySelectorAll('.comp-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  document.getElementById('f-comp').value = btn.dataset.val;
}
...
function addMed() {
  ...
  medicines.push({ id: nextId, name, dose, freq, time, comp, notes, taken: false, ic: nextId });
  nextId++;

  render();
  closeSheet();
  resetForm();
  showToast(`${name} — Compartiment ${comp} ajouté`);
}
...
function resetForm() {
  ...
  document.getElementById('f-comp').value = 'A';
  document.querySelectorAll('.comp-btn').forEach(b => b.classList.remove('active'));
  const first = document.querySelector('.comp-btn[data-val="A"]');
